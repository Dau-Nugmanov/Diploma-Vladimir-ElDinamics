...
InitialStyleSet - способ задания начальных условий
GPoints1, GPoints2 - значения временных реализаций
IntFPoints, IntBPoints - значения реализаций поверхностных мод
IntFModeEz, IntFModeHy - распределение поверхностных мод, по которым
  рассчитываются амплитуды
BettaX, BettaY - волновые числа
IntEnable - нужно ли рассчитывать поверхностные моды
LastAdded - количество значений во временной реализации
LastAddedI - количество значений во реализации поверхностных мод
SigmaXCoeffs, SigmaYCoeffs, SigmaZCoeffs, SigmaXSCoeffs, SigmaYSCoeffs
SigmaZSCoeffs, OneDivSigmaX, OneDivSigmaY, OneDivSigmaXS,
OneDivSigmaYS - значения коэффициентов для рассчета полей внутри
  поглощающих слоев}
